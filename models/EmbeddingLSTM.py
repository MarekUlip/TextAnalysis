from aliaser import Sequential,LSTM,Dense,Embedding,Dropout
...
class EmbeddingLSTMModel(Model):
    ...
    def get_compiled_model(self):
        ...
        if self.num_of_layers == 1:
            self.model.add(LSTM(self.num_of_neurons[0],activation=self.activation_functions[0]))
        else:
        ...
        get_embedding_matrix(self.num_of_words, self.embedding_dim, self.tokenizer.word_index)
        #self.model.layers[0].set_weights([self.embedding_matrix])
        self.model.layers[0].trainable = False
        return self.model
