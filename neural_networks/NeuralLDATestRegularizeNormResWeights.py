...
from dataset_loader.dataset_helper import Dataset_Helper, stp_wrds
...
history = autoencoder.fit(matrix,matrix,batch_size=32,epochs=1000,validation_split=0.1, verbose=2, callbacks=[EarlyStopping(monitor='val_loss', min_delta=0, patience=50, verbose=0, mode='auto', baseline=None, restore_best_weights=False)])
weight_in_tmp = autoencoder.get_weights()[0]
weight_out_tmp = autoencoder.get_weights()[2]
tst = autoencoder.get_weights()
blob = np.array([])

weight_in_tmp = weight_in_tmp.transpose()
...
params = [['l1','l1'],['l2','l2']]
for param in params:
    log_writer.add_log('Testing combination {}-{}'.format(param[0],param[1]))
    weight_in = normalize(weight_in_tmp,param[0],axis=0)
    weight_out = normalize(weight_out_tmp,param[1],axis=0)
    ...
    topic_words_in_max = get_extremes(weight_in,num_of_topics,num_of_important_words,reverse_word_map,True,'topic_words_in_max{}-{}'.format(param[0],param[1]),log_writer,dataset_helper.get_dataset_name())
    topic_words_out_max = get_extremes(weight_out,num_of_topics,num_of_important_words,reverse_word_map,True,'topic_words_out_max{}-{}'.format(param[0],param[1]),log_writer,dataset_helper.get_dataset_name())
    ...
    test_model(documents, labels, neural_lda_in, log_writer,'neural_lda_in{}-{}'.format(param[0],param[1]))
    test_model(documents, labels, neural_lda_out, log_writer,'neural_lda_out{}-{}'.format(param[0],param[1]))
    #test_model(documents, labels, neural_lda_combined, log_writer,'neural_lda_combined')


    measureCoherence(topic_words_in_max,log_writer,model.dictionary,documents,'neural_in_max{}-{}'.format(param[0],param[1]),dataset_helper.get_dataset_name())
    measureCoherence(topic_words_out_max,log_writer,model.dictionary,documents,'neural_out_max{}-{}'.format(param[0],param[1]),dataset_helper.get_dataset_name())
    ...
    plot_clustering_chart(neural_lda_out,False,documents,log_writer,'neural_topic_out{}-{}'.format(param[0],param[1]),dataset_helper.get_dataset_name())
    plot_clustering_chart(neural_lda_in,False,documents,log_writer,'neural_topic_in{}-{}'.format(param[0],param[1]),dataset_helper.get_dataset_name())
