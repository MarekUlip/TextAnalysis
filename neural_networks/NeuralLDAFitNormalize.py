...
test_name = simpledialog.askstring(title="Test Name",
                                  prompt="Insert test name:",initialvalue='LDATestsFitNorm{}'.format(norm_to_use))
...
output_row = Dense(num_of_words,activation='sigmoid',kernel_regularizer=keras.regularizers.l1_l2(regularization))(encoder)
...
for i in range(500):
    ...
    for b_ind in range(len(matrix)//batch_size-1):
        autoencoder.train_on_batch(matrix[b_ind*batch_size:(b_ind+1)*batch_size],matrix[b_ind*batch_size:(b_ind+1)*batch_size])
        #autoencoder.fit(matrix,matrix,batch_size=32,epochs=1,validation_split=0.1, verbose=2, callbacks=[EarlyStopping(monitor='val_loss', min_delta=0, patience=50, verbose=0, mode='auto', baseline=None, restore_best_weights=False)])
        weights = autoencoder.get_weights()
