...
params = [['sigmoidCrossTFIDF_L12In_0.001','sigmoid','categorical_crossentropy','tfidf',keras.regularizers.l1_l2(0.001),None],
...
['Kernel_L12Both_0.0001',keras.regularizers.l1_l2(0.001),keras.regularizers.l1_l2(0.0001)],
...
for param in params:
    ...
    matrix = tokenizer.texts_to_matrix(documents, mode=param[3])
    ...
    encoder= Dense(num_of_topics, activation='relu',activity_regularizer=param[4])(input_row)#
    #encoder= keras.layers.LayerNormalization()(encoder)
    #decoder = Dense(int(num_of_words/2), activation='relu')(encoder)
    output_row = Dense(num_of_words,activation=param[1],activity_regularizer=param[5])(encoder)

    autoencoder = Model(input_row,output_row)
    autoencoder.summary()
    #autoencoder.compile(optimizer='adadelta', loss='mse', metrics=['accuracy'])
    autoencoder.compile(optimizer='adam', loss=param[2],metrics=['accuracy'])#optimizer='adadelta', loss='mse', metrics=['accuracy'])
    #autoencoder.compile(optimizer='adadelta', loss='categorical_crossentropy', metrics=['accuracy'])
    history = autoencoder.fit(matrix,matrix,batch_size=32,epochs=1000,validation_data=(matrix,matrix), verbose=2, callbacks=[EarlyStopping(monitor='val_loss', min_delta=0, patience=100, verbose=0, mode='auto', baseline=None,
