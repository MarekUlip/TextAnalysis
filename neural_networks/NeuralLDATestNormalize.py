...
encoder = keras.layers.LayerNormalization()(input_row)
#encoder = Dense(int(num_of_words/2), activation='relu')(input_row)
encoder= Dense(num_of_topics, activation='relu', activity_regularizer=keras.regularizers.l1_l2(regularization))(encoder)#
...
history = autoencoder.fit(matrix,matrix,batch_size=32,epochs=1000,validation_split=0.1, verbose=2, callbacks=[EarlyStopping(monitor='val_loss', min_delta=0, patience=1000, verbose=0, mode='auto', baseline=None, restore_best_weights=False)])
