...
test_name = simpledialog.askstring(title="Test Name",
                                  prompt="Insert test name:",initialvalue='LDATestsCustomReg')
...
from topic_regularizer import TopicRegularizer
print(tf.executing_eagerly())
regularization = 0.001
input_row = Input(shape=(num_of_words,))
#encoder = Dense(int(num_of_words/2), activation='relu')(input_row)
encoder= Dense(num_of_topics, activation='relu', activity_regularizer=TopicRegularizer(dictionary,corpus,num_of_topics,num_of_important_words,reverse_word_map))(input_row)#
#encoder= keras.layers.LayerNormalization()(encoder)
#decoder = Dense(int(num_of_words/2), activation='relu')(encoder)
output_row = Dense(num_of_words,activation='sigmoid', activity_regularizer=TopicRegularizer())(encoder)
