from neural_networks.aliaser import Sequential,LSTM,Dense,Dropout
...
class LSTMModel(Model):
    ...
    def get_uncompiled_model(self):
        ...
        if self.num_of_layers == 1:
            self.model.add(LSTM(self.num_of_neurons[0],input_shape=(1, self.num_of_words),activation=self.activation_functions[0]))
        else:
            self.model.add(LSTM(self.num_of_neurons[0],input_shape=(1, self.num_of_words),return_sequences=True,activation=self.activation_functions[0]))
